...
export default function RiskTable({ summaries }: Props) {
  ...
  return (
    <div className="rounded-2xl bg-slate-900/60 border border-slate-800/60 overflow-hidden">
      ...
      {/* Column headers */}
      <div className="grid grid-cols-12 gap-3 px-6 py-3 border-b border-slate-800/40 text-[10px] font-semibold text-slate-600 uppercase tracking-widest">
      ...
      <div className="divide-y divide-slate-800/40">
        {sorted.map((s, i) => {
          ...
          const railColor =
            s.status === 'critical' ? 'bg-red-500' : 'bg-amber-500'
          ...
          return (
            <div
              key={s.id}
              className={`grid grid-cols-12 gap-3 px-6 py-4 items-center hover:bg-slate-800/30 transition-colors duration-100 ${rowBg} ${isTopRisk ? 'relative' : ''}`}
            >
              {isTopRisk && (
                <div className={`absolute left-0 top-0 bottom-0 w-0.5 ${railColor}`} />
              )}
              ...
              <div className="col-span-4 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${actualBarColor} rounded-full ${actualGlow}`}
