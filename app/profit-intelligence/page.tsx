...
export default function ProfitIntelligencePage() {
  ...
  async function runCalculation() {
    setCalculating(true)
    setInsight(null) // reset insights when recalculating
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm">Drink-level cost, margin, and profit for {fmtDate(periodStart)}–{fmtDate(periodEnd)}.</p>
        </div>
        {lastCalcAt && (
      ...
      {!loading && !hasData && (
        <div className="bg-slate-900/40 border border-slate-800/60 border-dashed rounded-2xl p-12 text-center">
          ...
          <p className="text-slate-600 text-sm mt-1 max-w-xs mx-auto">
            Set a cost per oz on your inventory items, then click Calculate Profit.
          </p>
            ...
            <SpotlightCard
              ...
              secondary={[
                lowestMargin?.estimated_cost != null && lowestMargin.quantity_sold > 0
                  ? `$${(lowestMargin.estimated_cost / lowestMargin.quantity_sold).toFixed(2)}/drink cost`
                  : null,
                ...
                .map((row, i) => {
                  ...
                  return (
                    <div key={row.id ?? i} className="flex items-center gap-3">
                      <p className="text-xs text-slate-500 w-28 shrink-0 truncate">{row.menu_item?.name ?? '—'}</p>
                      <div className="flex-1 relative h-5 flex items-center">
                        {/* Revenue bar */}
                        <div
                          className="absolute inset-y-0 left-0 rounded bg-slate-700/60"
                          style={{ width: `${revPct}%` }}
                        />
                        {/* Profit bar */}
                        <div
                          className="absolute inset-y-0 left-0 rounded bg-emerald-500/55"
                          style={{ width: `${profPct}%` }}
                        />
                      </div>
            ...
            <div className="px-5 py-5">
              {insightLoading ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-slate-500 text-sm">Analyzing {summaries.length} drinks…</p>
                </div>
              ) : insight ? (
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mt-0.5">
                      Analyzes your {summaries.length} drinks — top performers, margin gaps, and recommendations.
                    </p>
                  </div>
                  <button
                    onClick={generateInsight}
              ...
              {insightError && (
                <p className="text-red-400 text-xs mt-3">{insightError}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="hidden sm:block bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden">
              <div className="grid grid-cols-[1fr_3.5rem_5rem_5.5rem_5rem] px-5 py-2.5 border-b border-slate-800 bg-slate-800/30">
                {['Drink','Sold','Revenue','Est. Profit','Margin'].map((h, i) => (
