...
export async function POST(req: NextRequest) {
  try {
    ...
    const email = `${clean}@barguard.app`

    const origin = req.headers.get('origin') || 'https://barguard.vercel.app'
    const redirectTo = `${origin}/reset-password`

    // Generate recovery link — also confirms the user exists
    ...
    // Look up the contact email via user_id → business
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px;background:#0f172a;color:#e2e8f0;border-radius:12px">
          ...
          <p style="color:#94a3b8;margin:0 0 24px">Hi <strong style="color:#e2e8f0">@${clean}</strong>, click the button below to set a new password.</p>
